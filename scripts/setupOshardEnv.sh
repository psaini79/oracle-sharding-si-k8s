...
setupSysShardSql="/tmp/setapp.sql"
...
echo  "alter session enable shard ddl;" > ${setupSysShardSql}
echo  "create user app_schema identified by app_schema_password;" >> ${setupSysShardSql}
echo  "grant all privileges to app_schema;" >> ${setupSysShardSql}
echo  "grant gsmadmin_role to app_schema;" >> ${setupSysShardSql}
echo  "grant select_catalog_role to app_schema;" >> ${setupSysShardSql}
echo  "grant connect, resource to app_schema;" >> ${setupSysShardSql}
echo  "grant dba to app_schema;" >> ${setupSysShardSql}
echo  "grant execute on dbms_crypto to app_schema;" >> ${setupSysShardSql}
echo "CREATE TABLESPACE SET TSP_SET_1 using template (datafile size 100m autoextend on next 10M maxsize unlimited  extent management local segment space management auto);" >> ${setupSysShardSql}
echo "CREATE TABLESPACE SET LOBTS1;" >> ${setupSysShardSql}
echo "CREATE TABLESPACE products_tsp datafile size 100m autoextend on next 10M maxsize unlimited extent management local uniform size 1m; "  >> ${setupSysShardSql}
echo "CONNECT app_schema/app_schema_password;" >> ${setupSysShardSql}
echo "ALTER SESSION ENABLE SHARD DDL;" >> ${setupSysShardSql}
echo "CREATE SHARDED TABLE Customers (CustId      VARCHAR2(60) NOT NULL, FirstName   VARCHAR2(60), LastName    VARCHAR2(60), Class       VARCHAR2(10), Geo         VARCHAR2(8),CustProfile VARCHAR2(4000),Passwd      RAW(60),CONSTRAINT pk_customers PRIMARY KEY (CustId),CONSTRAINT json_customers CHECK (CustProfile IS JSON)) TABLESPACE SET TSP_SET_1  PARTITION BY CONSISTENT HASH (CustId) PARTITIONS AUTO;" >> ${setupSysShardSql}
echo "CREATE SHARDED TABLE Orders (OrderId     INTEGER NOT NULL,CustId      VARCHAR2(60) NOT NULL, OrderDate   TIMESTAMP NOT NULL,SumTotal    NUMBER(19,4),Status      CHAR(4), CONSTRAINT  pk_orders PRIMARY KEY (CustId, OrderId),CONSTRAINT  fk_orders_parent FOREIGN KEY (CustId)    REFERENCES Customers ON DELETE CASCADE  ) PARTITION BY REFERENCE (fk_orders_parent);" >> ${setupSysShardSql}
echo "CREATE SEQUENCE Orders_Seq;" >> ${setupSysShardSql}
echo "CREATE SHARDED TABLE LineItems (OrderId     INTEGER NOT NULL,CustId      VARCHAR2(60) NOT NULL,ProductId   INTEGER NOT NULL,Price       NUMBER(19,4),Qty         NUMBER,CONSTRAINT  pk_items PRIMARY KEY (CustId, OrderId, ProductId),CONSTRAINT  fk_items_parent FOREIGN KEY (CustId, OrderId)    REFERENCES Orders ON DELETE CASCADE  ) PARTITION BY REFERENCE (fk_items_parent);" >> ${setupSysShardSql}
echo "CREATE DUPLICATED TABLE Products (ProductId  INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,Name       VARCHAR2(128),DescrUri   VARCHAR2(128),LastPrice  NUMBER(19,4)) TABLESPACE products_tsp;"  >> ${setupSysShardSql}
print_message "cat ${setupSysShardSql}"
executeSQL "$cmd1"  "$localconnectStr" "sqlScript"
